...
같은 문을 써도 버튼이 크론 실행을 제 실행으로 착각하지는 않습니다. dispatch 응답에 `runId`가 실려 오고, 버튼은 그때부터 `?runId=`로 그 실행만 지목해 추적합니다. 최근 `workflow_dispatch` 실행 하나를 보는 경로(`handleRefresh`의 `runId` 없는 분기)로 내려가는 것은 두 경우뿐입니다 — 응답에 `runId`가 없을 때(REST 문서상으로는 204 No Content가 정상이라 언제든 그럴 수 있습니다), 그리고 갱신 중에 페이지를 새로 열어 `resumeRefresh`가 상태를 되찾을 때입니다. 그때는 크론 실행을 보고 진행/완료를 표시할 수 있는데, 수집 자체는 정상이고 표시만 어긋납니다.
...
- 셀렉트로 세트를 갈아탄다. 내보내기/불러오기로 다른 기기에 옮긴다(같은 이름은 덮어쓰고 새 이름은 더한다).
