...
fetch("../collector/data/index.json")
  .then((response) => { if (!response.ok) throw new Error("수집된 CSV가 없습니다. collector.js를 먼저 실행하세요."); return response.json(); })
...
async function applyFilters() {
  ...
  try {
    const loaded = (await Promise.all(files.map((file) => fetch(`../collector/data/${file.path}`).then((response) => response.ok ? response.text() : "").then(parseCsv)))).flat();
...
function parseCsv(text) { const lines = parseLines(text.replace(/^\uFEFF/, "")); const header = lines.shift() || []; return lines.map((cells) => Object.fromEntries(header.map((key, index) => [key, cells[index] || ""]))).map((row) => ({ ...row, files: safeJson(row.files, []) })); }
