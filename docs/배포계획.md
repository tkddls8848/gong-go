# dev 브랜치 클라우드플레어 배포 계획

`dev` 브랜치(수집기 + CSV 뷰어)를 Cloudflare에 올려 여러 사람이 브라우저로 접근할 수 있게 하는 계획.
...
두 앱은 브랜치로 분리되어 있다. `main` 브랜치 루트 앱은 API를 실시간 프록시하고, `dev` 브랜치는 미리 수집한 CSV를 읽는다.

| 브랜치 | 위치 | 역할 | 실행 환경 |
|---|---|---|---|
| `main` | `/`(루트) | 나라장터 API 실시간 조회 앱 | Pages + Functions (배포 완료) |
| `dev` | `collector/` | 공공데이터 API 수집기 | 로컬 Node |
| `dev` | `downloader`·`converter`·`analyzer/` | 첨부 다운로드 → 문서 변환 → ECR 추출 | 로컬 Node |
| `dev` | `data/` | 모든 단계의 공용 데이터 디렉터리 | — |
| `dev` | `public/` | 수집된 CSV 뷰어 | 로컬 정적 서버 |
...
1. **데이터가 Git에 없다.** 루트 `.gitignore`가 `data/`를 제외하므로 Git 기반 Pages 빌드로는 CSV가 올라가지 않는다. (추적 파일 12개 확인)
...
3. **접근 통제가 없다.** `main` 브랜치의 `functions/_middleware.js` 게이트는 해당 Pages 프로젝트에만 적용된다. 새 프로젝트에는 별도로 넣어야 한다.
...
- 새 Pages 프로젝트 `gong-go-archive`, 프로덕션 브랜치 `dev`, 빌드 출력 디렉터리 `public`
- `functions/data/[[path]].js` 신규 작성
  ```js
...
| 항목 | 상태 |
|---|---|
| `.env` Git 커밋 이력 | **없음** (`git log --all` 확인 완료) |
| `data/` 추적 여부 | 제외됨 |
